...
#     for i in range(n):
#         # Utility for player i = sum_j valuations[i,j] * x[i,j]
#         ui = valuations[i, :] @ x[i, :]
#         utilities.append(ui)
#         # Ensure positivity so that log(ui) is feasible
...
#     # Solve the problem (no solver specified)
#     prob = cp.Problem(cp.Maximize(objective), constraints)
#     prob.solve()  # Will use the default solver that CVXPY can find
    
#     x_val = x.value
#     utils = np.array([float(valuations[i,:] @ x_val[i,:]) for i in range(n)])
    
#     # --- Extract dual variables for the last m constraints => raw prices ---
#     # These last m constraints are the resource constraints sum_i x[i,j] <= supply[j].
#     raw_prices_list = []
#     for constr in prob.constraints[-m:]:
#         raw_prices_list.append(constr.dual_value)
#     raw_prices = np.array(raw_prices_list, dtype=float)
    
#     # Avoid zero or negative prices, just in case
#     raw_prices = np.maximum(raw_prices, 1e-12)
    
#     # Normalize so that player 0 spends exactly her budget.
#     sum_for_player0 = raw_prices @ x_val[0, :]
#     alpha = budgets[0] / sum_for_player0
#     prices = alpha * raw_prices
    
#     # Spending for each player
#     spent = np.array([prices @ x_val[i, :] for i in range(n)])
